...
+ (NSArray *)itemsOfAbsolutePath:(NSString *)path{
    NSManagedObjectContext *context = kMOContext;
    NSPredicate *predicate = [NSPredicate predicateWithFormat:@"downloadTry = %@", path];
        ...
        if (content) {
            ...
            for (id text in content) {
                if ([text isKindOfClass:[NSString class]]) {
                    ...
                    [collector appendFormat:@"<body>%@</body>\n", text];
                    ...
                    [collector appendFormat:@"<_image>%@</_image>\n", directoryDocuments(path)];
